...
function Player:update(dt)
    ...
    -- Use math.max so the character position never cross the left edge.
    -- Use math.min so the character position never cross the right edge.
    self.x = math.max(0, self.x + self.dx * dt)
    self.x = math.min(VIRTUAL_WIDTH + self.map.tileWidth * 2, self.x + self.dx * dt)
end
...
function Player:calculateJumps()
    ...
    if self.dy < 0 then
        if self.map:tileAt(self.x, self.y).id ~= TILE_EMPTY or
            ...
            if playCoin then
                self.sounds['coin']:setVolume(0.25)
                self.sounds['coin']:play()
            elseif playHit then
                self.sounds['hit']:setVolume(0.25)
...
function Player:checkLeftCollision()
    if self.dx < 0 then
        ...
        -- Check for collitions with the flag on the left.
        if self.map:flagCollision(self.map:tileAt(self.x - 1, self.y)) or
            self.map:flagCollision(self.map:tileAt(self.x - 1, self.y + self.height - 1)) then
            -- Stop the music and change game state.
            self.map.music:stop()
            self.map.gameState = "victory"
        end
    end
end
...
function Player:checkRightCollision()
    if self.dx > 0 then
        ...
        -- Check for collition with the flag on the right.
        if self.map:flagCollision(self.map:tileAt(self.x + self.width, self.y)) or
            self.map:flagCollision(self.map:tileAt(self.x + self.width, self.y + self.height - 1)) then
            -- Stop the music and change game state.
            self.map.music:stop()
            self.map.gameState = "victory"
        end
